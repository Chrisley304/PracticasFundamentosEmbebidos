...
led = LED(17)
sensor = DistanceSensor(echo=27, trigger=22, max_distance=1, threshold_distance=0.2)
chat_id = ""
...
def subscribe_to_alerts(message):
    ...
    bot.send_message(chat_id, "¡Te has suscrito a las alertas de sensor de distancia!")
    ...
    while True:
        if sensor.distance * 100 < 5 and subscribed_users:
            ...
            for user_id in subscribed_users:
                bot.send_message(user_id, "ALERTA: Se ha detectado movimiento a menos de 10cm")
        else:
            led.off()
